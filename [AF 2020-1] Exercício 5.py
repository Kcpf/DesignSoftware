...
def mais_medalhas(lista_dicionarios):
    dic_nacionalidade = {}
    for each in lista_dicionarios:
        if each['nacionalidade'] not in dic_nacionalidade:
            dic_nacionalidade[each['nacionalidade']] = 0
        dic_nacionalidade[each['nacionalidade']] += each['ouro']
    
    ganhador = [k for k, v in sorted(dic_nacionalidade.items(), key=lambda item: item[1])][-1]
    return ganhador
